...
const ProfileSettings = () => {
  ...
  const [formState, setFormState] = useState({
    ...
    dob: currentAdvertiser.dob,
    imageURL: currentAdvertiser.imageURL,
  });
  ...
  const handleSubmit = async (event) => {
    ...
    if (
      currentAdvertiser.name != formState.name ||
      currentAdvertiser.dob != formState.dob ||
      currentAdvertiser.imageURL != formState.imageURL
    ) {
      const formData = new FormData();
      if (currentAdvertiser.name != formState.name) {
        formData.append("name", formState.name);
      }
      if (currentAdvertiser.dob != formState.dob) {
        formData.append("dob", formState.dob);
      }
      if (currentAdvertiser.imageURL != formState.imageURL) {
        formData.append("adImage", imageFile);
      }
      try {
        setIsLoading(true);
        const response = await axios.post(
          `http://localhost:5000/advertiser/upload/${currentAdvertiser._id}`,
          formData,
          {
            headers: {
        ...
        localStorage.setItem("advertiser", JSON.stringify(response.data));
  ...
  const handlePasswordChange = async () => {
    try {
      if (newPassword == newPasswordConfirm) {
        setErrorText("");
        await axios.put("http://localhost:5000/advertiser/change/password", {
          email: currentAdvertiser.email,
