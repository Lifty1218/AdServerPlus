...
const PublisherAssistance = () => {
  ...
  useEffect(() => {
    if (!location.state) {
      navigate("/admin/publisher_assistance");
    } else {
  ...
  useEffect(() => {
    if (publisher.id) {
      socket.emit("join", publisher.id);

      axios
        .get(`http://localhost:5000/publisher/messages/${publisher.id}`)
...
export default PublisherAssistance;
